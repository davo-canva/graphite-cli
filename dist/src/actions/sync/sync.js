...
async function pullTrunk(force, context) {
    ...
    context.splog.warn(`${chalk_1.default.blueBright(context.metaCache.trunk)} could not be fast-forwarded.`);
    if (force ||
        (context.interactive &&
            (await (0, prompts_1.default)({
                type: 'confirm',
                name: 'value',
                message: `Overwrite ${chalk_1.default.yellow(context.metaCache.trunk)} with the version from remote?`,
                initial: true,
            }, {
                onCancel: () => {
        ...
        context.splog.info(`${chalk_1.default.green(context.metaCache.trunk)} set to ${chalk_1.default.gray(context.metaCache.getRevision(context.metaCache.trunk))}.`);
    }
    else {
        throw new errors_1.KilledError();
    }
