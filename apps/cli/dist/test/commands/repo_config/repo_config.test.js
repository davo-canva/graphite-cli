...
for (const scene of [new basic_scene_1.BasicScene()]) {
    describe(`(${scene}): infer repo owner/name`, function () {
        ...
        it('Can infer cloned repos', () => {
            const match = (0, repo_config_spf_1.getOwnerAndNameFromURL)('https://github.com/withgraphite/graphite-cli.git');
            if (match === null) {
                chai_1.expect.fail('Match should not be null');
            }
        ...
        it('Can infer SSH cloned repos', () => {
            const match = (0, repo_config_spf_1.getOwnerAndNameFromURL)('git@github.com:withgraphite/graphite-cli.git');
            if (match === null) {
                chai_1.expect.fail('Match should not be null');
            }
            const { owner, name } = match;
        ...
        it('Can infer SSH cloned repos (with git@ configured separately)', () => {
            const match = (0, repo_config_spf_1.getOwnerAndNameFromURL)('github.com/withgraphite/graphite-cli.git');
            if (match === null) {
                chai_1.expect.fail('Match should not be null');
            }
            const { owner, name } = match;
