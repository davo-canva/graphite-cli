...
function composeEngine({ git, trunkName, currentBranchOverride, splog, noVerify, remote, restackCommitterDateIsAuthorDate, }) {
    ...
    return {
        ...
        getChangedFiles: (branchName) => {
            const meta = cache.branches[branchName];
            return git.getFilesChanged(!meta || meta.validationResult !== 'VALID'
                ? `${branchName}~`
        ...
        getDiff: git.getDiff,
        getStackDiff: (branchName) => {
            const meta = assertBranchIsValidOrTrunkAndGetMeta(branchName);
            return git.getDiff(meta.validationResult === 'TRUNK'
                ? `${branchName}~`
                : meta.parentBranchRevision, branchName);
        },
        getParentOrPrev: (branchName) => {
            const meta = assertBranchIsValidOrTrunkAndGetMeta(branchName);
...
exports.composeEngine = composeEngine;
