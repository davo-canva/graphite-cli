...
async function editDownstack(inputPath, context) {
    ...
    const branchesToRestack = context.engine.getRelativeStack(branchNames[0], scope_spec_1.SCOPE.UPSTACK);
...
async function promptForEdit(context) {
    const branchNames = context.engine.getRelativeStack(context.engine.currentBranchPrecondition, scope_spec_1.SCOPE.DOWNSTACK);
