...
async function splitCurrentBranch(args, context) {
    ...
    const children = context.engine.getRelativeStack(branchToSplit, scope_spec_1.SCOPE.UPSTACK_EXCLUSIVE);
...
async function splitByCommit(branchToSplit, context) {
    ...
    const readableCommits = context.engine.getAllCommits(branchToSplit, 'READABLE');
...
function getSplitByCommitInstructions(branchToSplit, context) {
    return [
        ...
        ...(context.engine.getPrInfo(branchToSplit)?.number
            ? [
                `If any of the new branches keeps the name ${chalk_1.default.cyan(branchToSplit)}, it will be linked to PR #${context.engine.getPrInfo(branchToSplit)?.number}.`,
            ]
            : []),
        ``,
...
async function splitByHunk(branchToSplit, context) {
    ...
    try {
        ...
        const defaultCommitMessage = context.engine
            ...
            .reverse()
            .join('\n\n');
        for (let unstagedChanges = context.engine.getUnstagedChanges(); unstagedChanges.length > 0; unstagedChanges = context.engine.getUnstagedChanges()) {
            context.splog.info(instructions);
            context.splog.newline();
