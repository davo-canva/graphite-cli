...
function getUntrackedBranchNames(context) {
    return context.engine.allBranchNames.filter((branchName) => !context.engine.isTrunk(branchName) &&
...
async function interactiveBranchSelection(opts, context) {
    ...
    const indexOfCurrentIfPresent = choices.findIndex((choice) => choice.value ===
        (opts.omitCurrentBranch
            ? context.engine.getParentPrecondition(context.engine.currentBranchPrecondition)
    ...
    if (args.short) {
        return [
            `${'│ '.repeat(args.indentLevel)}${'◯'}${args.skipBranchingLine || numChildren <= 2
                ...
                : args.reverse
                    ? '─┐'
                    : '─┘'}▸${args.branchName}${args.noStyleBranchName || context.engine.isBranchFixed(args.branchName)
                ? ''
