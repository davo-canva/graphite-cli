...
function restackBranches(branchNames, context) {
    ...
    while (branchNames.length > 0) {
        ...
        switch (result.result) {
            case 'REBASE_DONE':
                context.splog.info(`Restacked ${chalk_1.default.green(branchName)} on ${chalk_1.default.cyan(context.engine.getParentPrecondition(branchName))}.`);
                continue;
            case 'REBASE_CONFLICT':
                ...
                (0, print_conflict_status_1.printConflictStatus)(`Hit conflict restacking ${chalk_1.default.yellow(branchName)} on ${chalk_1.default.cyan(context.engine.getParentPrecondition(branchName))}.`, context);
                throw new errors_1.RebaseConflictError();
            case 'REBASE_UNNEEDED':
                context.splog.info(`${chalk_1.default.cyan(branchName)} does not need to be restacked${` on ${chalk_1.default.cyan(context.engine.getParentPrecondition(branchName))}`}.`);
                continue;
            default:
