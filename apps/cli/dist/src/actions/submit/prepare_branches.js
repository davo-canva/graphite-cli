...
async function getPRInfoForBranches(args, context) {
    ...
    for await (const branchName of args.branchNames) {
        ...
        const parentBranchName = context.engine.getParentPrecondition(action.branchName);
        submissionInfo.push({
...
async function getPRAction(args, context) {
    ...
    const parentBranchName = context.engine.getParentPrecondition(args.branchName);
    ...
    const shouldSkipUpdate = args.always === false && (args.dryRun || status === 'NOOP');
...
async function getPRCreationInfo(args, context) {
    if (args.editPRFieldsInline) {
        context.splog.newline();
        context.splog.info(`Enter info for new pull request for ${chalk_1.default.cyan(args.branchName)} ▸ ${chalk_1.default.blueBright(context.engine.getParentPrecondition(args.branchName))}:`);
    }
    const submitInfo = {};
    try {
...
async function getPRUpdateInfo(args, context) {
    ...
    if (args.editPRFieldsInline) {
        context.splog.newline();
        context.splog.info(`Enter updated info for pull request for ${chalk_1.default.cyan(args.branchName)} ▸ ${chalk_1.default.blueBright(context.engine.getParentPrecondition(args.branchName))}:`);
        try {
            submitInfo.title = await (0, pr_title_1.getPRTitle)({
                branchName: args.branchName,
                editPRFieldsInline: args.editPRFieldsInline,
