...
function getBranchInfo(args, context) {
    ...
    const branchInfoLines = [
        `${args.displayAsCurrent
            ...
            : chalk_1.default.blueBright(args.branchName)} ${context.engine.isBranchFixed(args.branchName)
            ? ''
            : chalk_1.default.yellow(`(needs restack)`)}`,
        `${chalk_1.default.dim(context.engine.getAllCommits(args.branchName, 'COMMITTER_DATE')[0] ?? '')}`,
