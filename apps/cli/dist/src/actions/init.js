...
async function selectTrunkBranch(allBranchNames, context) {
    const inferredTrunk = context.engine.findRemoteBranch() ?? findCommonlyNamedTrunk(context);
    if (!context.interactive) {
        if (inferredTrunk) {
...
function findCommonlyNamedTrunk(context) {
    const potentialTrunks = context.engine.allBranchNames.filter((b) => ['main', 'master', 'development', 'develop'].includes(b));
...
async function branchOnboardingFlow(context) {
    ...
    await (0, checkout_branch_1.checkoutBranch)({ branchName: context.engine.trunk }, context);
