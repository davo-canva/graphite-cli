...
function displayBranchesInternal(opts, context) {
    ...
    const currentChoice = {
        display: `${'  '.repeat(opts.indent ?? 0)}↱ $ ${opts.branchName}${context.engine.isBranchFixed(opts.branchName)
            ? ''
...
function logShortClassic(context) {
    context.splog.info(displayBranchesInternal({ branchName: context.engine.trunk, highlightCurrentBranch: true }, context)
        .map((b) => b.display)
        .join('\n'));
}
