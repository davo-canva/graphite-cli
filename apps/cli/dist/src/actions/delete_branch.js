...
function deleteBranchAction(args, context) {
    ...
    const branchesToRestack = context.engine.getRelativeStack(args.branchName, scope_spec_1.SCOPE.UPSTACK_EXCLUSIVE);
...
function isSafeToDelete(branchName, context) {
    ...
    const reason = prInfo?.state === 'CLOSED'
        ...
        : prInfo?.state === 'MERGED'
            ? `${chalk_1.default.green(branchName)} is merged into ${chalk_1.default.cyan(prInfo?.base ?? context.engine.trunk)}`
            : context.engine.isMergedIntoTrunk(branchName)
                ? `${chalk_1.default.green(branchName)} is merged into ${chalk_1.default.cyan(context.engine.trunk)}`
                : context.engine.isBranchEmpty(branchName)
                    ? `${chalk_1.default.yellow(branchName)} is empty`
                    : undefined;
