...
async function getAction(args, context) {
    ...
    context.splog.info(context.engine.pullTrunk() === 'PULL_UNNEEDED'
...
async function getBranchesFromRemote(args, context) {
    ...
    for (const [index, branchName] of args.downstack.entries()) {
        ...
        else if (context.engine.getParentPrecondition(branchName) !== parentBranchName) {
...
async function handleSameParent(args, context) {
    ...
    switch (fetchChoice) {
        ...
        case 'OVERWRITE':
            context.engine.checkoutBranchFromFetched(args.branchName, args.parentBranchName);
