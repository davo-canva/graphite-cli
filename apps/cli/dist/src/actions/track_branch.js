...
function getPotentialParents(args, context) {
    return context.engine.allBranchNames
        .filter((potentialParentBranchName) => context.engine.isTrunk(potentialParentBranchName) ||
        ((!args.onlyTrackedParents ||
            context.engine.isBranchTracked(potentialParentBranchName)) &&
            context.engine.isDescendantOf(args.branchName, potentialParentBranchName)))
        .sort((left, right) => {
        return left === right
            ? 0
...
async function trackStack(args, context) {
    ...
    if (choices.length === 0) {
        throw new errors_1.ExitFailedError(`No possible parents for this branch. Try running \`git rebase ${context.engine.trunk} ${branchName}\``);
    }
...
async function trackBranch(args, context) {
    ...
    if (!context.engine.isTrunk(args.parentBranchName) &&
