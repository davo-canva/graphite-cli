...
copyPathToOutput("apps/gti-server/dist");
...
copyPathToOutput("node_modules/ws");
...
copyPathToOutput(path.join("apps/gti", clientBuildDir));
const gtiScript = path.join(outDir, "run-gti");
const gtiBat = path.join(outDir, "run-gti.bat");
fs.copyFileSync("./release/run-gti.template.sh", gtiScript);
fs.copyFileSync("./release/run-gti.template.bat", gtiBat);
fs.chmodSync(gtiScript, 0o755);
fs.chmodSync(gtiBat, 0o755);

console.info(
  `You can run GTI at: ${process.platform === "win32" ? gtiBat : gtiScript}`
);
